# Bitergia Trainings

The goal of this repository is to produce a training session written down, full of information and rich in details.


## Areas of Training

[General Overview](overview.md)
* [Menu Sections](menu_sections.md)

[Filtering](filtering.md)
* [Time Picker](timepicker.md)
* [Sticky Filters](sticky_filters.md)
* [Drag & Drop](drag_drop.md)
* [Table Results Filtering](tables.md)

[Export and Sharing](export_sharing.md)
* [Share Button](share_button.md)
* [Drill Down](drill_down.md)
* [Export to CSV](export_csv.md)
* [Request Metadata](request_metadata.md)
